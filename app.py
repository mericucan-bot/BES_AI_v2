...
st.set_page_config(page_title="BES Fon Önerisi", page_icon="images/favicon_32.png", layout="wide")
...
with tab2:
    ...
    _risk_desc = {
        ...
        "dengeli":     "Orta risk, büyüme + koruma dengesi",
        "agresif":     "Yüksek risk, maksimum büyüme hedefi",
    }
    _risk_alloc = {
    ...
    # === ERKEN HESAP (status_band + öneriler) ===
    ...
    if total_value > 0:
        ...
        _class_tl, _unmapped_tl, _funds_in_class = {}, {}, {}
    ...
    if total_value > 0:
        ...
        cost = {"total_cost_tl": 0, "switch_count": 0, "turnover_pct": 0, "exceeds_monthly_limit": False}

    _class_weights = (
        {k: v / _mapped_total for k, v in _class_tl.items()} if _mapped_total > 0 else {}
    )
    try:
        _sig = compute_significance(result, None, _class_weights, target_weights or {}, cost)
    except Exception:
        _sig = {"score": 0, "level": "quiet", "reasons": []}

    _existing_contrib = _active_data_t2.get("monthly_contribution_tl", 0) if _active_data_t2 else 0
    _contrib = analyze_contribution(_existing_contrib or None)

    if _class_weights:
        _top_cls = max(_class_weights, key=_class_weights.get)
        _top_w = _class_weights[_top_cls]
        _top_name = _asset_labels.get(_top_cls, _top_cls)
        _conc_v = f"%{_top_w*100:.0f} {_top_name}"
        _conc_tone = "alert" if _top_w >= 0.70 else ("warn" if _top_w >= 0.50 else "normal")
        _conc_s = "tek sınıf yüksek" if _top_w >= 0.50 else "dengeli"
    else:
        _conc_v, _conc_s, _conc_tone = "—", "veri yok", "normal"

    _sig_score = _sig.get("score", 0)
    _sig_level = _sig.get("level", "quiet")
    _sig_tone = "alert" if _sig_level == "action" else ("warn" if _sig_level == "notable" else "good")
    _sig_s = {"action": "aksiyon", "notable": "dikkat", "quiet": "sakin"}.get(_sig_level, "")

    _contrib_gap = _contrib.get("match_gap", 0) or 0
    if _contrib.get("at_cap"):
        _dk_v = f"{_contrib['annual_match']:,.0f} TL"
        _dk_s, _dk_tone = "tavan dolu", "good"
    else:
        _dk_v = f"{_contrib_gap:,.0f} TL"
        _dk_s, _dk_tone = "kaçan katkı", ("warn" if _contrib_gap > 0 else "normal")

    status_band([
        {"k": "Portföy Değeri", "v": format_tl(total_value) if total_value else "0 TL",
         "s": regime_info.get("label", ""), "tone": "normal"},
        {"k": "Bu Ay Önem", "v": f"{_sig_score}/100", "s": _sig_s, "tone": _sig_tone},
        {"k": "Yoğunlaşma", "v": _conc_v, "s": _conc_s, "tone": _conc_tone},
        {"k": "Devlet Katkısı", "v": _dk_v, "s": _dk_s, "tone": _dk_tone},
    ])

    _s_durum, _s_duzenle, _s_analiz = st.tabs(
        ["Durum & Öneriler", "Portföyü Düzenle", "Analiz"]
    )

    with _s_durum:
        if total_value == 0:
            st.warning("Portföy değeri 0 TL. 'Portföyü Düzenle' sekmesinden fon tutarlarını gir.")
        else:
            st.markdown(f"""
            ...
            if _user_rp:
                ...
                st.caption("Risk profilini belirleyerek kişiselleştirilmiş öneriler alabilirsin. 'Portföyü Düzenle' sekmesindeki anketi doldur.")
            if _contrib["at_cap"]:
                st.success(
    ...
    with _s_duzenle:
        ...
        with st.expander(f"{active_pf_name} — Düzenle", expanded=True):
        ...
        with st.expander(_expander_label, expanded=not bool(_existing_profile)):
            if _existing_profile:
                st.success(f"**Senin profilin: {_risk_labels[_existing_profile]}** — {_risk_desc[_existing_profile]}")
                _cur_alloc = _risk_alloc[_existing_profile]
                _alloc_str = "  |  ".join(f"**{_asset_labels[k]}** %{v}" for k, v in _cur_alloc.items())
                st.caption(f"Önerilen dağılım: {_alloc_str}")
                st.write("---")
                st.write("Profili güncellemek için soruları tekrar yanıtla:")

            _q1 = st.radio(
                "**S1: Yatırım süren ne kadar?**",
                ["1-3 yıl (Kısa vade)", "3-7 yıl (Orta vade)", "7+ yıl (Uzun vade)"],
                key="rp_q1", index=None,
            )
            _q2 = st.radio(
                "**S2: Portföyün %20 değer kaybetse ne yaparsın?**",
                ["Hepsini satarım", "Beklerim", "Daha fazla alırım"],
                key="rp_q2", index=None,
            )
            _q3 = st.radio(
                "**S3: Yatırım deneyimin?**",
                ["Yeni başlıyorum", "Biraz bilgim var", "Deneyimliyim"],
                key="rp_q3", index=None,
            )
            _q4 = st.radio(
                "**S4: Aylık gelirine göre BES katkın?**",
                ["Gelirimin %5'inden az", "%5-10 arası", "%10'dan fazla"],
                key="rp_q4", index=None,
            )
            _q5 = st.radio(
                "**S5: Getiri beklentin?**",
            ...
            if st.button("Profilimi Belirle", key="rp_submit", type="primary"):
                if None in [_q1, _q2, _q3, _q4, _q5]:
                    ...
                    _total_score = sum(_score_map[a] for a in [_q1, _q2, _q3, _q4, _q5])
                    if _total_score <= 8:
                        _new_profile = "muhafazakar"
                    elif _total_score <= 12:
                        _new_profile = "dengeli"
                    else:
                    ...
                    st.success(f"**{_risk_labels[_new_profile]}** — {_risk_desc[_new_profile]}")
                    _alloc_res = _risk_alloc[_new_profile]
                    st.write("**Önerilen portföy dağılımı:**")
                    _rp_cols = st.columns(len(_alloc_res))
                    for _ci, (_ast, _pct) in enumerate(_alloc_res.items()):
                        _rp_cols[_ci].metric(_asset_labels[_ast], f"%{_pct}")
                    st.rerun()
...
""", unsafe_allow_html=True)
        ...
        pass
    st.caption("Geçmiş performans, öneri karnesi ve enflasyon etkisi.")

    # === ÖĞRENME + GEÇMİŞ (status_band için erken) ===
    learning = LearningEngineV2()
    ...
    _t3_months = len(history_df) if not history_df.empty else 0
    if not history_df.empty and len(history_df) >= 2:
        _t3_first = history_df["total_value"].iloc[0]
        _t3_last  = history_df["total_value"].iloc[-1]
        _t3_ret   = (_t3_last / _t3_first) - 1 if _t3_first > 0 else 0
        if "real_value" in history_df.columns and history_df["real_value"].notna().any():
            _rv0 = history_df["real_value"].iloc[0]
            _rv1 = history_df["real_value"].iloc[-1]
            _t3_real_ret = (_rv1 / _rv0) - 1 if _rv0 and _rv0 == _rv0 else None
        else:
            _t3_real_ret = None
    else:
        _t3_ret = None
    ...
    status_band([
        {"k": "Takip Süresi", "v": f"{_t3_months} ay" if _t3_months else "—",
         "s": "snapshot", "tone": "normal"},
        {"k": "Toplam Getiri",
         "v": f"%{_t3_ret*100:+.1f}" if _t3_ret is not None else "—",
         "s": "nominal",
         "tone": "good" if (_t3_ret is not None and _t3_ret > 0) else (
             "alert" if (_t3_ret is not None and _t3_ret < 0) else "normal")},
        {"k": "Öğrenme", "v": f"{total_obs} gözlem",
         "s": "öğreniyor" if total_obs >= 6 else "öğrenme aşaması",
         "tone": "good" if total_obs >= 6 else "warn"},
        {"k": "Reel Getiri",
         "v": f"%{_t3_real_ret*100:+.1f}" if _t3_real_ret is not None else "—",
         "s": "enflasyon arındırılmış", "tone": "normal"},
    ])

    _s_perf, _s_karne, _s_enf = st.tabs(
        ["Performans", "Karne & Backtest", "Enflasyon"]
    )
...
with tab4:
    ...
    if not ml_summary_path.exists():
        status_band([
            {"k": "Model", "v": "eğitilmemiş", "s": "python main.py --ml-train", "tone": "warn"},
        ])
        st.warning(
        ...
        status_band([
            {"k": "Model", "v": str(best_model).upper(),
             "s": f"eğitim {run_date}", "tone": "normal"},
            {"k": "Sinyal (IC)", "v": f"{best_ic:.2f}", "s": quality_text,
             "tone": "good" if best_ic >= 0.4 else ("warn" if best_ic >= 0.2 else "alert")},
            {"k": "Yön Doğruluğu", "v": f"%{best_dir_acc*100:.0f}",
             "s": "dir_acc", "tone": "normal"},
            {"k": "Analiz Edilen Fon", "v": f"{fund_count}",
             "s": "fon sayısı", "tone": "normal"},
        ])

        st.caption(f"Son eğitim: {run_date} | {quality_emoji} {quality_text}")

        _s_tahmin, _s_karsilastir, _s_model = st.tabs(
            ["Tahminler", "Fon Karşılaştır", "Model Detayı"]
        )
...
<div style="display:flex; align-items:center; justify-content:space-between;
      ...
      <div style="font-weight:700; color:{_rp_clr}; font-size:1.05rem;">{_rp_arr} %{_rp_ret*100:+.1f}</div>
        ...
        with _s_model:
            st.markdown('<div class="section-heading">Model Karşılaştırma</div>', unsafe_allow_html=True)
...
- **IC:** Bilgi katsayısı — tahmin sıralamasının gerçek sıralamayla uyumu (yüksek = iyi, 0.3+ güçlü)
            ...
            st.markdown('<div class="section-heading">Model Neye Bakıyor? (Feature Importance)</div>', unsafe_allow_html=True)
